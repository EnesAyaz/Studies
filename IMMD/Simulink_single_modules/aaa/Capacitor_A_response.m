...
%%
k=length(SAIA_mag);
capA_real=SAIA_mag(1:k)'.*capA_phA(1:k).* exp(1i*SAIA_angle(1:k))';+SBIB_mag(1:k)'.*capA_phB(1:k).* exp(1i*SBIB_angle(1:k))' ...
    +SCIC_mag(1:k)'.*capA_phC(1:k).* exp(1i*SCIC_angle(1:k))';
capA=abs(capA_real);
figure();
