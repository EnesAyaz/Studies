...
% yyaxis(axes1,'left');
plot(copper_width_X,(P_eddy_tot)/1e3,'LineStyle',"-",'Color','r' ...
    ,'Marker','x','MarkerSize',10,'MarkerFaceColor','r');
hold on;
plot(copper_width_X,(Pdc_tot)/1e3,'LineStyle',"-",'Color','b' ...
     ,'Marker','o','MarkerSize',5,'MarkerFaceColor','b');
hold on;
plot(copper_width_X,(P_eddy_tot+Pdc_tot)/1e3,'LineStyle',"-",'Color','k' ...
...
grid minor;
%%
A=[round(((P_eddy_tot)/1e3)'*10)/10 , ...
round(((Pdc_tot)/1e3)'*10)/10 , ...
round(((P_eddy_tot+Pdc_tot)/1e3)'*10)/10 ];
