...
void EPwm4(void); // Complementary 2 tane pwm üretiliyor. Bu 2PWM frequency veya phase e göre EWPM1 e göre deðiþiyor.
...
#define switching_frequency 176000 // Switching frequency 150khz olarak seçildi.
...
void EPwm4(void)
{
#if TZ_TURNOFF
    EALLOW;
            EPwm4Regs.TZSEL.bit.OSHT1 = 1; //1. event benim butona basýp kapatmam
    ...
    EPwm4Regs.CMPA.bit.CMPA = EPwm5Regs.TBPRD/2; // EPWM2A için %50duty
    EPwm4Regs.CMPB.bit.CMPB = EPwm5Regs.TBPRD/2; // EPWM2B için %50duty
#if FREQCONT
...
__interrupt void adca1_isr(void) // Hizli okuma akým okumasý falan
  ...
  cnt=cnt+1;
    ...
    ////////////////////////
     ...
     Vavg= 3300*(Vsum)/(4095.0*7.35);
         ...
         Verror=Vavg-Vset;
              AdcaResults_freq=AdcaResults_freq+p_cont*Verror;
